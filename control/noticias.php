...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tInfo">
  ...
  <?
  $result=mysql_query("SELECT*FROM ".$nombreTablaNoticias." WHERE publicacion!='' ORDER BY id_publicacion DESC");
  $i=0;
  ...
  <tr>
    ...
    <td><?=formatoFecha($fila["creacion"])?></td>
    <td><input type="button" class="botonesConf" name="btModificar<?=$i?>" id="btModificar<?=$i?>" value="Modificar" onclick="modificar('<?=$fila["id_publicacion"]?>');" /></td>
    <td><input type="button" class="botonesAlert" name="btEliminar<?=$i?>" id="btEliminar<?=$i?>" value="Eliminar" onclick="eliminar('<?=$fila["id_publicacion"]?>');" /></td>
  </tr>
  <?
  	$i++;
  }
